...
module.exports = class extends think.Service {
  constructor(db = null) {
    ...
    this.modelVisualComponentsIns = db
      ? think
          .model("visualComponents", {}, think.config("custom.appModules.web"))
          .db(db)
      : think.model(
          "visualComponents",
          {},
    ...
    this.modelVisualOrgIns = db
      ? think
          .model("visualOrg", {}, think.config("custom.appModules.web"))
          .db(db)
      : think.model("visualOrg", {}, think.config("custom.appModules.web"));
   ...
   * @returns {Promise<*>}
   */
  async initDevWorkspace(component_id, options = {}) {
    const {user_name = ''} = options;
    // 查询组件的内容
    ...
    try {
      ...
      if (!(await fs.exists(target.npmrcFile))) {
        await fs.writeFile(
          target.npmrcFile,
          source.npmrcTpl()
        );
      ...
      if (isFirstInit) {
        ...
        if (type==EnumComponentType.type_ht) {
          const { typeId } = compData;
          const res = await this.sceneService.getSceneById(typeId);
          const sceneInfo = res[0];
          if (!(await fs.exists(target.srcComponentJsFile))) {
            await fs.writeFile(
              target.srcComponentJsFile,
              source.srcComponentJsTplForHt(component_mark,sceneInfo)
            );
          }
          //添加3D文件
          await this.helperService.copyDir(
            path.resolve(source.publicDir),target.componentMarkPath+'/public'
          )
        }else{
          if (!(await fs.exists(target.srcComponentJsFile))) {
            await fs.writeFile(
  ...
  async getComponentInfoById(component_id) {
    ...
    return {
      ...
      git_lab_project_id: componentInfo.git_lab_project_id,
    };
  }
  async getSceneInfoById(sceneId) {
    const sceneInfo  = await this.sceneModelIns
      .where({sceneId})
      .find();

    return {
      dirPath: sceneInfo.dirPath,
   ...
   * @return {Promise<*>}
   */
  async compileComponent(component_id, options = {}) {
    const {isProduction = false, user_id} = options;
  ...
  async mkComponentZip(component_id) {
    ...
    // 编译组件包
    const compileResp = await this.compileComponent(component_id, {isProduction: true});
   ...
   */
  async copyComponent(origin_component_id, target_component_id, options = {}) {
    const { user_name } = options;
