...
module.exports = class extends think.Service{
	constructor(){
		...
		this.permissionService = think.service('rbac/permission', 'common', this.modelUserRoleIns.db());
	}
    ...
    async delRole(account_id, role_ids) {
        return await this.modelUserRoleIns.transaction(async () => {
            ...
            // 删除角色菜单权限关系
            const delPermissionRes = await this.permissionService.delMenuPermissionByIds(account_id, role_ids, EnumPermissionSubjectType.role);
            if (think.isError(delPermissionRes)) throw delPermissionRes;
