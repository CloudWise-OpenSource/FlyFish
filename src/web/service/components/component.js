...
module.exports = class extends think.Service{
    constructor(){
        ...
        this.modelComponentsOrg = think.model('componentsOrg', {}, think.config('custom.appModules.web'));
    ...
    async getComponentList() {
        ...
        const componentList = result.map(item => {
            item.org_mark = orgMap[item.org_id] && orgMap[item.org_id].org_mark || '';
            item.org_name = orgMap[item.org_id] && orgMap[item.org_id].name || '';
            return item;
        });
        return {component_list: componentList};
    }
  ...
  async updateFileOrDir(account_id, org_id, targetOrg_id, component_mark) {
    const organizeService = think.service('components/organize', think.config("custom.appModules.web"), this.modelComponentIns.db());
    ...
    async copyComponent(account_id, user_id, params){
        const devComponentIOService = think.service('components/devComponentIO', think.config("custom.appModules.web"), this.modelComponentIns.db());

        const { component_id, target_org_id, target_component_mark } = params;
        ...
        return await this.modelComponentIns.transaction(async () => {
            ...
            const copyRes = await devComponentIOService.copyComponent(component_id, target_component_id);
