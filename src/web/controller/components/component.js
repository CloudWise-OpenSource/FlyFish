...
module.exports = class extends Base{
    ...
    constructor(ctx) {
        super(ctx);
        this.componentService = think.service('components/component', think.config("custom.appModules.web"));
    }

    /**
     * @api {POST} /web/components/component/add 添加组件
    ...
    async addAction() {
        ...
        let { name, component_mark, org_id: org_id = 1, categories_id: categories_id = 1,type, typeId} = this.post();
    ...
    /**
     * @api {GET} /web/components/component/getPageList 获取组件分页列表
     * @apiGroup VComponentsManage
     * @apiVersion 1.0.0
    ...
    async getComponentListAction() {
        const result = await this.componentService.getComponentList();
    ...
    /**
     * @api {GET} /web/components/component/getDetail 获取单个组件分类详情
     * @apiGroup VComponentsManage
     * @apiVersion 1.0.0
    ...
    /**
     * @api {PUT} /web/components/component/update 更新组件分类
     * @apiGroup VComponentsManage
     * @apiVersion 1.0.0
    ...
    async updateAction() {
        const { account_id } = await this.getCacheUserInfo();
        const { component_id, name} = this.post();
        const result = await this.componentService.updateComponent(account_id, component_id, {name});
    ...
    /**
     * @api {DELETE} /web/components/component/delete 删除组件
     * @apiGroup VComponentsManage
     * @apiVersion 1.0.0
     * @apiDescription 删除组件
     *
    ...
    /**
     * @api {PUT} /web/components/component/shelf 上下架组件
     * @apiGroup VComponentsManage
     * @apiVersion 1.0.0
     * @apiDescription 组件上下架
     *
     * @apiHeader (请求头部) {String} Content-Type application/json
     * @apiParam (入参) {Array} component_id 组件ID
     * @apiParam (入参) {String} shelf_status 组件上下架状态 [on || off] 
     *
     * @apiSuccessExample Success-Response:
     *  {
	 *      "code": 0,
     ...
     */
     async shelfAction() {
        const { account_id } = await this.getCacheUserInfo();
        const { component_id, shelf_status } = this.post();
        const result = await this.componentService.updateComponent(account_id, component_id, { shelf_status });

        if (shelf_status === 'on') {
            const devComponentIOService = think.service('components/devComponentIO', think.config("custom.appModules.web"));
            await devComponentIOService.shelfComponent(component_id);
        }
    ...
    /**
     * @api {GET} /web/components/component/download 组件下载
     * @apiGroup VComponentsManage
     * @apiVersion 1.0.0
     * @apiDescription 组件下载
     *
    ...
    /**
     * @api {GET} /web/components/component/downloadComponentCode 组件源代码下载
     * @apiGroup VComponentsManage
     * @apiVersion 1.0.0
    ...
    /**
     * @api {GET} /web/components/component/downloadComponentCode 组件源代码导出
     * @apiGroup VComponentsManage
     * @apiVersion 1.0.0
    ...
    /**
     * @api {POST} /web/components/component/copy 复制组件
     * @apiGroup VComponentsManage
     * @apiVersion 1.0.0
     * @apiDescription 复制组件
     *
    ...
    async copyAction() {
        const { account_id, user_id } = await this.getCacheUserInfo();
        let { component_id, target_org_id: target_org_id = 1, target_component_mark } = this.post();

        const result = await this.componentService.copyComponent(account_id, user_id, {component_id, target_org_id, target_component_mark });
        if (think.isError(result)) return this.fail(result.message);
