...
public class JsonPathUtil {
    ...
    public static void objectResponse(){
        ...
        Object val = context.read("$.data[?]", Filter.filter(Criteria.where("price").lt(500)));
    ...
    public static void listResponse(){
        ...
        try {
            String objectRespJson = getListRespJson();
            JsonNode jsonNode = JsonUtils.objectMapper.readTree(objectRespJson);
        ...
        Object val4 = context.read("$.data[?]",Filter.filter(Criteria.where("category").is("reference")));
