        ...
        for (ResourceComponentDto dto : resourceComponentDtoList) {
            ...
            if (file.exists()) {
                ...
                if (null != covers && covers.length > 0) {
                    String coverPath = File.separator + covers[0].getName();
                    comp.setCover(lcap_www_relative_path + COMPONENTS + File.separator + componentId + File.separator + version + COMPONENT_RELEASE + coverPath);
                }
            }
