...
const CodeDevelop = observer((props) => {
  ...
  useEffect(() => {
    if (developingData) {
      componentId.current = developingData.id;
    }
  }, [developingData]);

  const initDevelopingData = async () => {
    if (!developingData) {
      const id = window.location.hash.match(/\/app\/code-develop\/(.*)/)[1];
      const res = await getDetailDataService({ id });
      if (res && res.code === 0) {
        setDevelopingData(res.data);
      }
    }
  };
  ...
  const installPackages = async () => {
    setInstalling(true);
    const res = await installPackagesService(
      developingData && developingData.id
    );
    if (res && res.code === 0) {
      setInstalling(false);
    ...
    <div className={styles.wrap}>
      ...
      >
        ...
        >
          ...
          <Spin
            tip='正在进行编译...'
            spinning={compileSping}
            wrapperClassName='vscodeSpinWrap'
          >
            <iframe
              className={styles.vscodeFrame}
              name='vscode'
              src={`${window.FLYFISH_CONFIG.vscodeAddress}/?folder=${
                window.FLYFISH_CONFIG.vscodeFolderPrefix
              }/components/${developingData && developingData.id}/v-current`}
              frameBorder={0}
            ></iframe>
          </Spin>
        </div>
        <div
          className={layout == 'row' ? styles.rowBar : styles.colBar}
          onMouseDown={(e) => {
            setMoveMode(true);
          }}
          onMouseUp={(e) => {
        ...
        >
          ...
          <iframe
            ...
            name='preview'
            src={`${window.FLYFISH_CONFIG.wwwAddress}/components/${
              developingData && developingData.id
            }/v-current/editor.html?random=${previewRandom}`}
            frameBorder={0}
          ></iframe>
        </div>
        <div
