...
import { Input, Select, Button, message, Row, Col, Collapse, Icon, Form } from '@chaoswise/ui';
...
export default Form.create({ name: 'FORM_IN_PROJECT_MODAL' })(function EditProjectModal({
      ...
      if (!err) {
        ...
        dataUsability(sendData, (res) => {
          ...
          if (res.code === successCode && res.data.available) {
            ...
            message.error(res.msg || '连接失败,请重试！');
          }
        }).catch((res) => {
          setConnectState(false);
        });
        ...
        if (activeData && activeData.schemaType) {
          ...
          changeData(sendData, (res) => {
            ...
            if (res.code === successCode) {
              ...
              message.error(res.msg || '数据源编辑失败,请检查参数再进行保存！');
            }
          }).catch(() => {
            setSaveLoading(false);
          });
  ...
  return (
    <Form style={{ width: '80%' }} layout="vertical" labelAlign="left" initialvalues={projectData || {}}>
          ...
          <Panel header="请求体" key="1" style={{ border: 'none' }}>
            <Row gutter={20}>
              <div className={styles.menu}>
                <div
                  className={styles.menuItem + (!checkMenu ? ' ' + styles.activeItem : '')}
                  onClick={() => {
                    setCheckMenu(false);
                    editor.destroy();
                    let editFirst = new JSONEditor(editContainer.current, editorOptions);
            ...
            <Row gutter={20}>{<div ref={editContainer} id="editor"></div>}</Row>
          </Panel>
        </Collapse>
      </Form.Item>
