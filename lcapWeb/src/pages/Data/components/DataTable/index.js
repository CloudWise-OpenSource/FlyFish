...
import { Input, Select, Popconfirm, Checkbox, Icon, Row, Table, InputNumber, message, Collapse, Tooltip } from "@chaoswise/ui";
...
const ProjectDetail = observer(({ tableData = [], newTable, columnsTitle, title, setActiveContent, canChangeValue }) => {
    ...
    const expandedRowRender = record => {
        if (!record.state) {
            return (
                <>
                    <Select defaultValue="String" value={record.type || 'String'} style={{ width: 120, marginRight: '40px' }} onChange={(e) => { changeDetail(record.key, e, true); }}>
                    ...
                    return (
                        <>
                            ...
                            {inputBodyType && inputBodyType === recode.key && <Input placeholder={`请输入${columnsTitle[1]}`}
                                ...
                                defaultValue={text || ''}
                                onPressEnter={()=>{changeOneValue(recode);}}
                                onBlur={() => {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text, recode) => {
                if (recode.state) {
                    return (
                        <>
                            {!inputBodyType && <a style={{color:text?'#40a9ff':'gray'}} onClick={() => { setInputBodyType(true); }}>{text || '添加Header值'}</a>}
                            {inputBodyType && ['string', 'boolean'].includes(recode.type.toLowerCase()) ? <Input placeholder="请输入Header值"
                                ref={deleteInput}
                                style={{ height: 26, width: '100%' }}
                                onChange={(e) => {
                                    setBodyInputTitle(e.target.value);
                                ...
                                defaultValue={text || ''}
                                onPressEnter={()=>{
                                     if (recode.type.toLowerCase() == 'boolean' && bodyInputTitle !== 'true' && bodyInputTitle !== 'false') {
                                        message.error('该值只支持布尔类型，请重新输入');
                                        let oldArr = _.cloneDeep([...parseTableData]);
                                        let index = oldArr.findIndex(item => item.key === recode.key);
                                        let endItem = oldArr[index];
                                        endItem.default = '';
                                        setActiveContent(oldArr);
                                        deleteInput.current.setValue('');
                                        return;
                                    }
                                    changeValue(recode);
                                }}
                                ...
                                <InputNumber
                                    style={{ height: 26, width: '100%' }}
                                    precision={recode.type.toLowerCase() == 'double' ? null : 0}
                                    defaultValue={Number(text)}
                                    min={0}
                                    onBlur={() => {
                                        changeValue(recode);
                                    }}
                                    onChange={(e) => setBodyInputTitle(e)}
                                />}
                        </>
                    );
                } else {
        ...
        },
        {
            ...
            key: 'type',
            render: (text, record) => {
                return <Select disabled defaultValue={record.type} value={record.type || 'String'} style={{ width: 80, marginRight: '40px' }} onChange={(e) => { changeDetail(record.key, e, true); }}>
                    <Option value="String">文本</Option>
                    <Option value="Int">整数</Option>
                    <Option value="Double" >浮点数</Option>
                    <Option value="Boolean">布尔值</Option>
            ...
            render: (text, record) => {
                if (!record.state && !record.required) {
                    return (
                        ...
                        >
                            <Tooltip title="删除">
                                <Icon style={{ fontSize: '16px', cursor: 'pointer' }} type="minus-circle" />
                            </Tooltip>
                        </Popconfirm>
                    );
                }
        ...
        <div style={{ marginBottom: '20px' }} >
            ...
            >
                <Panel header={title} key="1" style={{ border: 'none' }}>
                    <Table tableLayout='fixed' style={{ marginTop: '20px' }} columns={newTable ? NewColumns : columns} dataSource={parseTableData} pagination={false} rowKey='key' expandedRowRender={newTable == 1 && expandedRowRender}
