 ...
 * @LastEditTime: 2022-09-23 11:33:57
 */
...
const ComponentDevelop = observer((props) => {
  const columns = [
    ...
    },
    {
      ...
      render: (text) => {
        return (
          <img
            src={window.FLYFISH_CONFIG.snapshotAddress + text}
            width={100}
            height={40}
              ...
              <div className={styles.btnWraper}>
                <div
                  onClick={() => {
                    ...
                    setShowRecord(false);
                    props.history.push({
                      pathname: `/app/${record.id}/code-develop`,
  ...
  const exportCode = async (id, onProgress) => {
    ...
    axios
      .get(
        `${window.FLYFISH_CONFIG.apiDomain}/components/export-source-code/${id}`,
        {
          responseType: 'blob',
          timeout: 0,
        }
