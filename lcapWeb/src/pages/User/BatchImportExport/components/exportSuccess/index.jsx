...
const ExportSuccess = observer((props) => {
  ...
  const exportCode = async (onProgress) => {
    ...
    if (componentOrApp === 'app') {
      param = {
        ids: selectedApp,
        applicationExportType: exportRadioData,
        componentExportType: exportCheckboxData,
      };
      requestUrl = `${window.FLYFISH_CONFIG.devServerPrefix}/resources/export/applications`;
    } else {
      param = {
        ids: selectedComponents,
        componentExportType: exportCheckboxData,
      };
      requestUrl = `${window.FLYFISH_CONFIG.devServerPrefix}/resources/export/components`;
    }
    axios
  ...
  return (
    <>
      {!exportSuccess && exportResAbnormal ? (
        <div
          className={styles.importProgress + ' ' + styles.exportSuccessStyle}
        >
        ...
        <Result
          ...
          extra={[
            ...
            <Button key='backToExportOrImport' onClick={backToExportOrImport}>
              返回导入导出管理
            </Button>,
          ]}
        />
