...
import { Table, Tag, Icon, Select, Input, Form, TreeSelect,message } from '@chaoswise/ui';
    ...
    return editing ? (
      <Form.Item style={{ margin: 0 }}>
        {form.getFieldDecorator(dataIndex, {
          ...
          return (inputElement = node)}} onPressEnter={save} onBlur={save} />)}
      </Form.Item>
    ) : (
      <div
...
const ImportConfig = observer((props) => {
  ...
  const categoryOnchange = (param)=>{
    ...
    newData.forEach(item => {
      if(item.id === value.three.id){
        item.category = value.one
        item.subCategory = value.two
        item.categoryName = param.label
      }
    });
    setImportTableData(newData)
    ...
    newData.forEach(item => {
      item.components.forEach(element => {
        if(element.id === value.three.id){
          element.category = value.one
          element.subCategory = value.two
          element.categoryName = param.label
        }
      });
    });
    setImportTableData(newData)
  ...
  const versionOnchange = (event,record) => {
    ...
    if(versionValue.length > 0){
      ...
      let newData = _.cloneDeep(toJS(importTableData))
      newData.forEach(item => {
        if(item.id === record.id && item.versionFlag !== ''){
          item.versionFlag = ''
          item.version = ''
  ...
  const appsVersionOnchange = (event,record) => {
    versionValue = event.currentTarget.value
    if(versionValue.length > 0){
        clearTimeout(timer)
        timer = setTimeout(()=>{
      ...
      newData.forEach(item => {
        item.components.forEach(element => {
          if(element.id === record.id && element.versionFlag !== ''){
            element.versionFlag = ''
            element.version = ''
        ...
        <>
          {
            ...
            >
              <Input
                ...
                defaultValue={record.versionEdit?record.version:''}
                ...
                componentClassifyTreeData.map((v,k)=>{
                  return {
                    ...
                    children:v.children.map((v1,k1)=>{
                      return {
                        title:v1.name,
                        value:JSON.stringify({one:v.id,two:v1.id,three:record}),
                        key:k+'-'+k1,
                      }
                    })
                  }
