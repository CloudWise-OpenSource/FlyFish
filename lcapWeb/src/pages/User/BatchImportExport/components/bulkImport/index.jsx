...
const BulkImport = observer((props) => {
  ...
  const { setIsUseLastImportSource, setUploadSuccess, setIsAppHasComponent, setPreviouStepFlag, setComsInAppTableData, setImportTableData, setImportFailedMsg, setSelectedRows } = store
  ...
  const handleImport = () => {
    ...
    let errorFlag=false
    importTableData.some(item=>{
      if(item.versionFlag==='error'){
        errorFlag=true
      }
    })
    if (isComponent) {
      ...
      if (!isAppHasComponent && !previouStepFlag) {
        comsInAppTableData.forEach(item => {
          if (item.type !== 'common' && (item.projects.length === 0 || item.projectsName.length === 0)) {
            validataFlag = false
            message.error('请选择' + item.name + '组件所属项目')
          }
        });
        ...
        if (element.components !== null) {
          element.components.forEach(eleItem => {
            if (item.id === eleItem.id) {
              item.appName = item.appName && (item.appName !== element.name) ? (item.appName + '、' + element.name) : element.name
            }
          })
        }
      });
        ...
        {current === steps.length - 1 && (
          <>
            {isAppHasComponent ?
              ...
              : <Button type="primary" onClick={handleImportDebounce} key='components'>
                导入
              </Button>
            }
          </>
