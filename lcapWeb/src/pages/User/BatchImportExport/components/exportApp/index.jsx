...
const ImportComponent = observer((props) => {
  ...
  const onChangeCheckbox = (checkedList) => {
    setIndeterminate(
      !!checkedList.length && checkedList.length < listDataOnlineNum
    );
  ...
  const deleteApp = (value) => {
    ...
    setSelectedApp(
      selectedApp.filter((item) => {
        return item !== value.id;
      })
    );
    setCheckAll(false);
    setIndeterminate(checkedList.length > 1);
  };
