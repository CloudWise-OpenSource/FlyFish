...
const EnumAPI = {
    ...
    dvScreen_getDelPageList: _processAPI(
        '/web/visualScreen/screen/getDelPageList'
    ), // 获取已删除大屏分页列表
    dvScreen_undoDelete: _processAPI('/web/visualScreen/screen/undoDelete'), // 还原已删除大屏
    dvScreen_downloadScreen: _processAPI(
        '/web/visualScreen/screen/downloadScreen'
    ), // 下载大屏部署包
    dvScreen_getTagList: _processAPI(
        '/web/tag/visualScreenTag/getDetailByScreenId'
    ), // 根据大屏id获取对应标签列表
    ...
    dvComponents_getPageList: _processAPI(
        'web/components/component/getPageList'
    ), // 获取可视化组件分页列表
    dvComponents_getDetail: _processAPI(
        'web/components/component/getDetail'
    ), // 获取单个可视化组件详情
    dvComponents_addComponent: _processAPI(
        'web/components/component/add'
    ), // 添加可视化组件
    dvComponents_updateComponent: _processAPI(
        'web/components/component/update'
    ), // 更新可视化组件
    dvComponents_delComponent: _processAPI(
        'web/components/component/delete'
    ), // 删除可视化组件
    dvComponents_putaway: _processAPI(
        '/web/components/component/shelf'
    ), // 上架可视化组件
    dvComponents_downloadComponentCode: _processAPI(
        'web/components/component/downloadComponentCode'
    ), // 导入可视化组件源代码
    dvComponents_importComponentCode: _processAPI(
        'web/components/component/importComponentCode'
    ), // 下载可视化组件源代码
    dvComponents_copy: _processAPI('web/components/component/copy'), // 复制可视化组件源代码

    dvComponents_dev_initDevSpace: _processAPI(
        'web/components/devComponentIO/initDevWorkspace'
    ), // 初始化可视化组件开发空间
    dvComponents_dev_readDevFile: _processAPI(
        'web/components/devComponentIO/readDevFile'
    ), // 读取开发组件文件内容
    dvComponents_dev_saveDevFileContent: _processAPI(
        'web/components/devComponentIO/saveDevFileContent'
    ), // 保存开发组件文件内容
    dvComponents_dev_addDevFileOrDir: _processAPI(
        'web/components/devComponentIO/addDevFileOrDir'
    ), // 添加开发组件文件或目录
    dvComponents_dev_updateDevFileOrDir: _processAPI(
        'web/components/devComponentIO/updateDevFileOrDir'
    ), // 更新开发组件文件或目录
    dvComponents_dev_delDevFileOrDir: _processAPI(
        'web/components/devComponentIO/delDevFileOrDir'
    ), // 删除开发组件文件或目录
    dvComponents_dev_npmDevComponent: _processAPI(
        'web/components/devComponentIO/npmDevComponent'
    ), // npm install 开发组件
    ...
    dvComponents_dev_uploadFile: _processAPI(
        'web/components/devComponentIO/uploadFile'
    ), // 上传文件
    ...
    /* 权限管理 */
    permission_getMenuPermission: _processAPI(
        '/web/rbac/permission/getMenuPermission'
    ), // 获取菜单权限
    permission_disposeMenuPermission: _processAPI(
        '/web/rbac/permission/disposeMenuPermission'
    ), // 处理菜单权限
