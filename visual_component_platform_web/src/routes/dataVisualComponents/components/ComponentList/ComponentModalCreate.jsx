 ...
 */
 ...
 export default class ComponentModalCreate extends Component {
		 ...
		 this.setState({fetching: true}, () => {
			 if (component_id) {
				 T.request.all(doGetOrgAll(), doGetAllCategories(),doGetComponentDetail(component_id)).then(resp => {
					 ...
					 const {name, component_mark, categories_id, org_id, create_user_id} = componentResp.data;
					 this.setState({
						 fetching: false,
						 name,
						 component_mark,
				 ...
				 T.request.all(doGetOrgAll(),doGetAllCategories()).then(resp => {
					 ...
					 this.setState({
						 fetching: false,
						 componentCategories: categoriesResp.data,
						 categories_id: categoriesResp.data[0] ? categoriesResp.data[0].categories_id : null,
						 organize: organizeRespData,
						 org_id: organizeRespData[0] ? organizeRespData[0].org_id : null,
					 });
				 }, resp => {
					 this.setState({fetching: false});
					 T.prompt.error(resp.msg);
	  ...
	  */
	 saveComponent = () => {
		 const { name, component_mark, categories_id, org_id } = this.state;
		 ...
		 this.setState({ saving: true }, () => {
			 ...
			 if (isCreate) {
				 doAddComponent({ name, component_mark, categories_id, org_id ,type:EnumComponentType.type_common}).then(...thenParams);
			 }else {
				 doUpdateComponent({ name, categories_id, component_id, org_id }).then(...thenParams);
			 }
		 })
	 }
