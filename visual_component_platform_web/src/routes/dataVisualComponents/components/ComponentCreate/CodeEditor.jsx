...
export default class CodeEditor extends PureComponent {
	...
	*/
	...
	render() {
        const { user } = T.auth.getLoginInfo() || {}

		const { component_id, component_mark, org_mark } = this.props;
		const { npmInstalling } = this.state;   // 编辑器的高度
		const codeServerUrl = window.ENV.coderDomain+"/?component_id="+component_id+ "&user_id=" + user.user_id +"&folder=/data/app/flyfish-v2.0/visual_component_platform/www/static/dev_visual_component/dev_workspace/"+ org_mark +"/" + component_mark;
		
		// const codeServerUrl = window.ENV.coderDomain+"/?component_id="+component_id+"&folder=/Users/martel/Documents/工作/cloudWiseLowCode/visual_component_platform_server/www/static/dev_visual_component/dev_workspace/"+ org_mark +"/" + component_mark;
		return (
